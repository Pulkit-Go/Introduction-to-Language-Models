...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    mu_B = (1/np.sum(np.square(bias_axis))) * np.dot(mu, bias_axis) * bias_axis
    mu_orth = mu - mu_B

    # Step 4: Use equations (7) and (8) to compute e_w1B and e_w2B (≈2 lines)
    e_w1B = (1/np.sum(np.square(bias_axis))) * np.dot(e_w1, bias_axis) * bias_axis
    e_w2B = (1/np.sum(np.square(bias_axis))) * np.dot(e_w2, bias_axis) * bias_axis
        
    # Step 5: Adjust the Bias part of e_w1B and e_w2B using the formulas (9) and (10) given above (≈2 lines)
    corrected_e_w1B = (e_w1B - mu_B) * np.sqrt(abs(1 - np.sum(np.square(mu_orth)))) * (np.square(1/np.linalg.norm(e_w1 - mu_B - mu_orth)))
    corrected_e_w2B = (e_w2B - mu_B) * np.sqrt(abs(1 - np.sum(np.square(mu_orth)))) * (np.square(1/np.linalg.norm(e_w2 - mu_B - mu_orth)))
